...
@section('title')
    {{trans_choice('general.edit',1)}}  {{trans_choice('general.campaign',1)}} {{trans_choice('general.type',1)}}
@endsection
@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title">{{trans_choice('general.edit',1)}}  {{trans_choice('general.campaign',1)}}  {{trans_choice('general.type',1)}}</h3>

            <div class="box-tools pull-right">

            </div>
        </div>
        {!! Form::open(array('url' => url('pledge/campaign_type/'.$pledgetype->id.'/update'), 'method' => 'post', 'class' => 'form-horizontal')) !!}
        <div class="box-body">
            <div class="form-group">
                <div class="col-sm-12">
                    {!! Form::label('name',trans_choice('general.name',1),array('class'=>'control-label')) !!}
                    {!! Form::text('name',$pledgetype->name, array('class' => 'form-control', 'placeholder'=>"",'required'=>'required')) !!}
                </div>
            </div>
			<div class="form-group">
                <div class="col-md-12">
					...
					<select class="fund form-control" name="fund">
						...
						@if(!empty($funds))
							@foreach($funds as $fund)
								<option value="{{$fund->id}}" {{$pledgetype->fund == $fund->id ? 'selected' : ''}}>{{$fund->name}}</option>
							@endforeach
						@endif
					</select>
                </div>
			...
			<div class="form-group">
				<div class="col-md-12">
					<table class="table">
						...
						@if(!empty($pledge_type_estimation))
							@foreach($pledge_type_estimation as $key => $estimation)
