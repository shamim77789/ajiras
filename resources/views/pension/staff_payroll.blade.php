...
@section('title'){{$user->first_name}} {{$user->last_name}}-{{trans_choice('general.pension',1)}}
@endsection
@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title">{{$user->first_name}} {{$user->last_name}}-{{trans_choice('general.pension',1)}}</h3>
        ...
        <div class="box-body">
            <div class="table-responsive">
                <table id="view-repayments"
                    ...
                    <tbody>
                    @foreach($user->pension as $key)
                        <tr>
                            <td>
                                {{$key->date}}
                            </td>
                            <td>
                                {{\App\Helpers\GeneralHelper::single_pension_total_pay($key->id)}}
                            </td>
                            <td>
                                {{\App\Helpers\GeneralHelper::single_pension_total_deductions($key->id)}}
                            </td>
                            <td>
                                {{\App\Helpers\GeneralHelper::single_pension_total_pay($key->id)-\App\Helpers\GeneralHelper::single_pension_total_deductions($key->id)}}
                            </td>
                            ...
                            <td>
                                <div class="btn-group-horizontal">
                                    <a type="button" class="btn-xs bg-blue"
                                       href="{{url('pension/'.$key->id.'/payslip')}}"
                                ...
                                <div class="btn-group-horizontal">
                                    @if(Sentinel::hasAccess('pension.delete'))
                                        <a type="button" class="btn-xs bg-purple"
                                           href="{{url('pension/'.$key->id.'/edit')}}">{{trans_choice('general.view_modify',1)}}</a><a
                                                type="button" class="btn-xs bg-navy margin delete"
                                                href="{{url('pension/'.$key->id.'/delete')}}">{{trans_choice('general.delete',1)}}</a>
                                    @endif
                                </div>
                            </td>
                        </tr>
