				...
				<select class="form-control select2 expense_type" name="expense_type">
					...
					@if(!empty($expense_type))
						@foreach($expense_type as $type)
							<option value="{{$type->id}}" {{$payroll->expense_type == $type->id ? 'selected' : ''}}>{{$type->name}}</option>
						@endforeach
					@endif
				</select>
            </div>
                              ...
                              <td width="50%" class="bg-gray">
                                 <table width="100%" id="gross_pay">
                                    <tbody>
                                       <tr>
                                          <td width="50%" class="cell_format">
                                             <div class="margin"><b>{{trans_choice('general.pension_fund',1)}}</b></div>
                                          </td>
                                          <td width="50%" class="cell_format">
                                             <div class="margin text-bold">
												 <select class="form-control pension_fund" name="pension_fund" onchange="refresh_totals()">
													 <option value="0" data-employer_contribution="0">Select Pension Fund</option>
													 @if(!empty($pension_fund))
													 	@foreach($pension_fund as $fund)
													 		<option value="{{$fund->id}}" data-employer_contribution="{{$fund->amount}}" data-type="{{$fund->type}}" {{$payroll->pension_fund == $fund->id ? 'selected' : ''}}>{{$fund->name}}</option>
													 	@endforeach
													 @endif
												 </select>
                                             </div>
...
@section('footer-scripts')
    ...
    <script>
        ...
        function refresh_totals(e) {
            ...
            var totalPaid = 0;
            var netPay = 0;
		    var fundType = $( ".pension_fund option:selected" ).attr('data-type');
	  	    var pensionFund = $( ".pension_fund option:selected" ).attr('data-employer_contribution');
	  	    
			pensionFund = parseInt(pensionFund);
			...
			if(fundType == '1') 
				pensionFund = (100 * pensionFund) / totalPay
			...
			document.getElementById("net_pay").value = (totalPay - totalDeductions - pensionFund).toFixed(2);
			document.getElementById("paid_amount").value = (totalPay - totalDeductions - pensionFund).toFixed(2);
        }
    </script>
@endsection